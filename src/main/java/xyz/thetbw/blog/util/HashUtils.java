...
public class HashUtils {
     ...
     * @return
     */
    public  String md5HashCode(InputStream fis,MD5EncodeAdapter adapter) {
        try {
     ...
     * @return
     */
    public  String md5HashCode32(InputStream fis,MD5EncodeAdapter adapter) {
        try {
            //拿到一个MD5转换器,如果想使用SHA-1或SHA-256，则传入SHA-1,SHA-256
            MessageDigest md = MessageDigest.getInstance("MD5");
     ...
     * @return
     */
    public int Sha1HashCode32(byte[] bytes){
        try {
    ...
    public interface MD5EncodeAdapter{
         public void run(byte[] temp, int lenght);
    }
